...
void Student_init(Student *ST , s32 index)
{
    ...
    ST->Entry->Subjects.subject_NO = 4; // not equal zero, to avoid division by zero if misuse
    ST->Entry->Subjects.subjects_average = -1;
}
...
static void genrate_id (Student * ST , s32 index)
{
    ...
    ST->Entry->id[11] = ST->Entry->phone_number[5];
